...
| `WEB_DIST_DIR` | OPT | api | — | DEPLOY-001 / ADR-023. Directory of the built web client. When set, this app serves it at `/` and the client is same-origin, so no CORS applies to it. Unset it to run an API-only app with the client hosted elsewhere |
