...
|---|---|---|---|
| Framework | **NestJS 11** + **Fastify** adapter | `@nestjs/core`/`@nestjs/common`/`@nestjs/platform-fastify`/`@nestjs/testing` `11.2.5`; `fastify` overridden to **5.12.5** (pnpm `overrides`, Stage 5 CI-000: GHSA-3m5p-2c4r-xxw2 / GHSA-w2qp-rph6-63g4; `@nestjs/platform-fastify@11.2.5` declares 5.11.3) | NestJS 12 exists (`latest` 12.0.3, 11 is tagged `legacy`). **NestJS 11 is chosen because `nestjs-zod@5.5.0` peer-supports only `^10 \|\| ^11`**, and Zod 3 is mandated. Upgrading to 12 requires a Zod 4 migration ADR |
...
| ORM | **Prisma 6 with the Rust library engine** and the MariaDB driver adapter (ADR-022) | `prisma@6.19.3`, `@prisma/client@6.19.3`, `@prisma/adapter-mariadb@6.19.3` (**patched**, see below), `mariadb@3.5.4` (the adapter's own `mariadb@3.4.5` is overridden to 3.4.7 for GHSA-42r5-vhpq-m858 / GHSA-cqhc-2h57-wpxf / GHSA-g5xc-5w98-jfvm; dev-only `mysql2` → 3.24.4 and `deepmerge-ts` → 8.0.2 likewise, Stage 5 CI-000) | **Pinned to 6.x by the deployment target, not by preference** (ADR-022): the plan runs Node under LiteSpeed's Passenger wrapper, which cannot instantiate Prisma 7's WebAssembly query compiler. Prisma 6 plans queries with a native Rust engine while keeping driver adapters, so `initSql` still applies the ADR-014 session settings. `binaryTargets = ["native", "rhel-openssl-1.1.x"]` for CloudLinux EL8. `@prisma/adapter-mariadb@6.19.3` carries a one-line backport of an upstream v7 fix (`patches/@prisma__adapter-mariadb@6.19.3.patch`): v6 reads MariaDB's BINARY_FLAG, which is set for any `_bin` collation, so our `ascii_bin` ids were returned as `Bytes`. **Do not install `prisma@latest`**, and do not bump to 7.x until the API no longer runs under Passenger |
...
| Static web serving (Stage 6) | `@fastify/static` serves the built web client from the API process (DEPLOY-001, ADR-023) | `@fastify/static@10.1.4` (MIT) | Chosen over a hand-written handler because static file serving is where path traversal lives. Only loaded when `WEB_DIST_DIR` is set, so an API-only profile does not register it |
| Fastify (direct, Stage 4) | `fastify` declared directly by `packages/http-kit` for hook/request types, same version Nest resolves | `fastify@5.12.5` (MIT; workspace override, see above) | Avoids relying on a transitive dependency. 5.12 fails closed on numeric `trustProxy`, hence `TRUST_PROXY` (audit C-48) |
