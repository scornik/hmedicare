...
| ORM | **Prisma** with MariaDB driver adapter | `prisma@7.10.0`, `@prisma/client@7.10.0`, `@prisma/adapter-mariadb@7.10.0`, `mariadb@3.5.4` (the adapter's own `mariadb@3.4.5` is overridden to 3.4.7 for GHSA-42r5-vhpq-m858 / GHSA-cqhc-2h57-wpxf / GHSA-g5xc-5w98-jfvm; dev-only `mysql2` → 3.24.4 and `deepmerge-ts` → 8.0.2 likewise, Stage 5 CI-000) | Prisma 7 requires driver adapters, the `prisma-client` generator with `output`, and `prisma.config.ts`. **Do not install `prisma@latest`**: it resolves to `8.0.0-rc.15` |
