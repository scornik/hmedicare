...
| 22 | Static front-end | "Vite on Hostinger is always static"; `vite build` → `dist`; free SSL on every domain and subdomain. Each Node app is bound to one domain, and the product has one | **SUPERSEDED by ADR-023** | The Node app serves `dist` itself at `/` with a SPA fallback (`WEB_DIST_DIR`), so the client is same-origin and needs no second domain, no `.htaccess` rewrite and no CORS grant | Split to `app.<domain>` later: unset `WEB_DIST_DIR`, set `CORS_ALLOWED_ORIGINS`, build with `VITE_API_BASE_URL` | HOST-012 |
...
| HOST-012 | Static web | **Re-scoped by ADR-023.** The Node app serves the client, so there is no second site to deploy: on the deployed app confirm `/` returns the shell, a deep link such as `/patients/x` survives a refresh, `/assets/*` carries `immutable` caching, `/api/v1/*` still answers as the API, and a missing asset 404s rather than returning HTML | Deep links work; the fallback never shadows `/api`, `/health` or `/internal` | Split the client onto its own subdomain (configuration only) |
...
| HOST-002 | 2026-09-22 | PASS | Ashik (Claude Code, browser check of the deployed app) | The monorepo apps/api runs on the plan under LiteSpeed Passenger from a GitHub push-to-deploy. /health/live answers {status:ok, app:api, version:42bfb79f, uptimeSeconds:0} and /health/ready answers {status:ready, checks:{db:ok}}, so Node 24 boots the app, the native modules load (argon2 0.41.1 for the plan's glibc 2.28, Prisma 6 Rust query engine for rhel-openssl-1.1.x) and the app reaches MariaDB. Getting here took eight fixes, each recorded on main: the pnpm version guard, the argon2 pin, an ARGON2_TIME_COST floor of 2, the execute bit on turbo, an explicit PORT (Passenger does not inject on |